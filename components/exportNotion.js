...
export default function ExportNotion(props) {
  const [data, setData] = useState(props.data);
  const [uploadSuccess, setUploadSuccess] = useState(false);

  const handleUpload = async (e) => {
    fetch(`https://z3biu5oi2g.execute-api.us-west-2.amazonaws.com/Prod/addToNotion?s3Url=${props.data.fileurl}`, {
      method: 'POST'
    })
    .then((response) => {
